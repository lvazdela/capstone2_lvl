...
[^1]: The methods used do not impute categorical variables. In this particular case, there were only two cases with a missing element in these type of variables, thus I decided to omit them.
...
The four models tested were trained with both imputation methods [^2]. The summary of the accuracy obtained with each method is presented in table \@ref(tab:tbfinres), together with the 95% confidence interval. While the imputation method had little influence in the accuracy of the logistic regression and de kNN models, the CART and random forests algorithms tended to perform better with the median imputation, although the confidence intervals overlap. The algoritm that renders the best accuracy is the random forests with median imputation (`r acc_rfmd`).

[^2]: For the sake of clarity in the manuscript, I decided to omit the code. Please [use this link](https://raw.githubusercontent.com/lvazdela/capstone2_lvl/main/capstone2_lvl.R) to open the corresponding R Script file.
...
Finally, we analyze the performance of the random forests algorithm. It addresses the shortcomings of the CART models, but we lose interpretability. As expected, this machine learning technique gave the best results concerning accuracy. When the data imputed with the median was used, random forests had an accuracy of `r acc_rfmd` \@ref(tab:tbfinres). The best number of variables sampled as candidates at each split was `r btune_rf` (figure \@ref(fig:fmtrytune), left panel). Although I did not tune up the number of trees, the right panel of figure \@ref(fig:fmtrytune) shows that the numer used (150) was enough to stabilize the error.

(ref:fmtrytune) Random forest model. *Left panel*: effect of the number of randomly selected predictors on accuracy. *Right panel*: effect of the number of trees on the stabilization of the error.

```{r fmtrytune, fig.cap='(ref:fmtrytune)', fig.show='hold', out.width='50%'}
plot(fit_rf, xlab = 'Number of randomly selected predictors')
plot(fit_rf$finalModel, main = NULL)
```

Table \@ref(tab:tbvarimp) describes the overall variable importance calculated for the models with the best imputation method using the `varImp` function of the `caret` package. Only the 20 most important variables are shown for each model. It is worth noting the absence of d-dimer (dimd) in the logistic regression model, but with the other variables, although there is no agreement with respect to the order, the algorithms agree in general with the 5 most important variables. Interestingly, neither diabetes mellitus nor high blood pressure were considered imortant risk factors by these models.
